...
class AuthRepository(
    ...
    suspend fun logIn(email: String, password: String): ApiResponse<LogInResponse> {
        try {
            ...
            } else if (response.status.value == 417 || response.status.value == 401) {
    ...
    private suspend fun AuthRepository.normal200LogIn(response: HttpResponse): ApiResponse<LogInResponse> {
        val logInResponseDTO = castResponseToLogInResponse200DTO(response)
        val id = logInResponseDTO.data!!.id
        val sesionToken = logInResponseDTO.data.token
        val email = logInResponseDTO.data.email


        userPreferencesDataStore.setUserId(id = id)
        userPreferencesDataStore.setUserEmail(email)
        userPreferencesDataStore.setSessionToken(sesionToken)
        userPreferencesDataStore.setIsLoggedIn(true)
        userPreferencesDataStore.setIsFirstLogin(false)

        return logInResponseDTO
    }

    private suspend fun AuthRepository.firstLogIn(response: HttpResponse): ApiResponse<LogInResponse> {
        val logInResponseDTO = castResponseToFirstLogInResponseDTO(response)
        val id = logInResponseDTO.data!!.id
        val sesionToken = logInResponseDTO.data.token
        val email = logInResponseDTO.data.email
        ...
        userPreferencesDataStore.setSessionToken(sesionToken)
        userPreferencesDataStore.setUserId(id = id)
        userPreferencesDataStore.setIsFirstLogin(true)
        userPreferencesDataStore.setIsLoggedIn(true)
        return logInResponseDTO
    }

    private suspend fun castResponseToFirstLogInResponseDTO(response: HttpResponse): ApiResponse<LogInResponse> {
    ...
    private suspend fun castResponseToLogInResponse200DTO(response: HttpResponse): ApiResponse<LogInResponse> {
        val data = ApiResponse(
            success = true,
            data = response.body<LogInResponse>(),
            message = "Bienvenido a courselab"
        )
        return data
    }
